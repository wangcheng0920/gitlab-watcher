...
3. 通知层：CLI 模式采用"设备适配器优先 + `node-notifier` 兜底"策略；serve 模式支持通过 `FEISHU_WEBHOOK_URL` 切换为飞书自定义机器人通知，支持通过 `FEISHU_AT_USERS` 配置后在交互卡片后追加 @mention 文本消息
...
| `FEISHU_AT_USERS` | 否 | 飞书通知艾特用户 open_id（逗号分隔），配置后在交互卡片后追加一条 @mention 文本消息 |
...
- [x] 飞书通知 @mention 支持（卡片后追加 text 消息 + `<at>` 标签）
- [x] tag 文件名编码/解码一致性修复
