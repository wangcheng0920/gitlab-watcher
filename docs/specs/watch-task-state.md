...
1. `watcher.pid`：当前 watcher 进程号，仅用于避免重复启动 watcher；正常退出时应删除。
2. `pending/<tag>.md`：待处理任务文件。创建时允许为空文件。
3. `processing/<tag>.md`：正在监听中的任务文件。每次查询结果都写入该文件。
4. `archive/<status>/<tag>.md`：已完成归档文件，仅在终态通知后写入。
...
| `terminalStatus` | `string` | 仅在 `notify_error` 记录中出现，表示已确认的终态 |
...
表示目标流水线终态已经确认，但本次本地通知发送失败。

该状态仅应出现在 `processing/` 中，并且记录里必须同时保留：
...
6. `processing/success|failed|canceled -> processing/notify_error`
7. `processing/notify_error -> archive/success|failed|canceled`
...
通过 `pending/<tag>.md` 表达。任务可通过以下方式创建：

1. CLI 交互式或命令行创建（`pnpm task:create`）
2. REST API 创建（`POST /tasks`）
...
该记录写入后，文件仍保留在 `processing/`，等待下一次轮询继续处理。

### `notify_error`

当目标流水线已经进入终态，但通知发送失败时，在文件顶部追加一段 `status: notify_error` 记录。
...
后续轮询遇到顶部为 `notify_error` 的任务时，不再重新查询 GitLab，而是直接基于该记录重试通知。

### `success` / `failed` / `canceled`

当目标流水线首次进入终态且通知成功后，在文件顶部追加终态记录，然后把文件移动到对应的归档目录。

如果首次终态通知失败，则任务保留在 `processing/`，先写入 `notify_error`；待后续通知重试成功后，再追加终态记录并归档。
...
通知失败待重试的 `processing/5.2.13.41.md` 也可形如：
